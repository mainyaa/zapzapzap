...
gulp.task('launch', shell.task([
  options.dev ? 'cache/Atom.app/Contents/MacOS/Atom build --proxy-server=127.0.0.1:3000' : 'cache/Atom.app/Contents/MacOS/Atom ./dist/osx/' + options.appFilename + '/Contents/Resources/app/build --proxy-server=127.0.0.1:3000'
]));
...
gulp.task('copy', function () {
  ...
  gulp.src('./build/**')
    .pipe(debug())
    .pipe(gulp.dest('./dist/osx/' + options.appFilename + '/Contents/Resources/app/build'))
    .pipe(gulpif(options.dev, reload({
      stream: true
    })));
});
